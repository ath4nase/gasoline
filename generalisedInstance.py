...
class GeneralisedInstance :
    ...
    def add_noise(self, intensity : int) :
        ...
        while not valid :
            ...
            new_x = [tuple(self.x[i][l] + noise_x[i][l] for l in range(self.k)) for i in range(self.n)]
            new_y = [tuple(self.y[i][l] + noise_y[i][l] for l in range(self.k)) for i in range(self.n)]

            valid = True
            if any(new_x[i][l] < 0 or new_y[i][l] < 0 for l in range(self.k) for i in range(self.n)) :
                valid = False
...
class MyGenealisedModel :
    ...
    def _init_vars(self, inst : GeneralisedInstance) -> None:
        for i in range(self.k):
            self.alpha.append(self.gurobi_model.addVar(vtype = GRB.INTEGER, name=f"alpha_{i}", lb = -float('inf'), ub = float('inf')))
    ...
    def __init_constrs(self, inst : GeneralisedInstance) -> None :
        ...
        for l in range(self.k) :
            ...
            self.gurobi_model.addConstrs(
            (gp.quicksum(inst.x[i][l] * self.z[i,j] for i in range(inst.n) for j in range(k))
            - gp.quicksum(inst.y[j][l] for j in range(0, k-1)) <= self.beta[l] for k in range(1, inst.n + 1)))
            # prefix greater than Alpha
            self.gurobi_model.addConstrs(
            (gp.quicksum(inst.x[i][l] * self.z[i,j] for i in range(inst.n) for j in range(k))
            - gp.quicksum(inst.y[j][l] for j in range(0, k)) >= self.alpha[l] for k in range(1, inst.n + 1)))

    def initialize(self, inst : GeneralisedInstance) -> None :
        self._init_vars(inst)
        self.__init_constrs(inst)
        self.gurobi_model.setObjective(gp.quicksum(self.beta[l] - self.alpha[l] for l in range(self.k)), GRB.MINIMIZE)
...
def generate_instance_distinct(n : int, k : int, x_min : int, x_max : int, y_min : int, y_max : int) -> GeneralisedInstance :
    ...
    while not valid : 
        ...
        diff = tuple(sum([inst.x[i][l] for i in range(n)]) - sum([inst.y[i][l] for i in range(n-1)]) for l in range(k))
